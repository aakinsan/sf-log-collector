...
def get_event_log_files(
    ...
    for event in event_types:
        ...
        if event_log_file_record["totalSize"] == 0:
            sflc_logger.info(
                f"Log files not found for the {event} event type"
            )
        else:
            ...
            convert_csv_file_to_json(f"{event}.csv",
                                    downloaded_csv_file.content)
    ...
    sflc_logger.info(
                f"Completed Log file upload to Storage Bucket"
            )
...
if __name__ == "__main__":
    try:
        get_event_log_files(TOKEN_ENDPOINT, private_key, CLIENT_ID, AUDIENCE,
                            SUBJECT, event_types, TIME_INTERVAL,
                            GOOGLE_PROJECT_ID, CLOUD_STORAGE_BUCKET)
