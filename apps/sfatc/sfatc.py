...
def get_sf_audit_trails(token_endpoint: str, private_key: str, client_id: str, audience: str, subject: str, created_date: str, google_project_id: str, storage_bucket: str) -> None:
    """ Collects Audit trails and uploads to GCP storage"""

    # Setup TLS session to salesforce instance url
    tls_session = create_tls_session(token_endpoint, private_key, client_id,
                                     audience, subject)

    # Get audit trail object from salesforce
    audit_trail_object = tls_session.get(f"/services/data/v58.0/query?q=SELECT+Action+,+CreatedBy.Name+,+CreatedById+,+CreatedDate+,+DelegateUser+,+Display+,+Id+,+Section+FROM+SetupAuditTrail+WHERE+CreatedDate+=+{created_date}"
    )
    audit_trail_record = audit_trail_object.json()

    if audit_trail_record["totalSize"] == 0:
        sflc_logger.info(f"Audit trail not available for {created_date}")

    else:
        # Using a helper function to write audit logs to json file
        convert_audit_to_json(audit_trail_record)
...
if __name__ == "__main__":
    try:
        get_sf_audit_trails(TOKEN_ENDPOINT, private_key, CLIENT_ID, AUDIENCE,
                            SUBJECT, created_date, GOOGLE_PROJECT_ID, CLOUD_STORAGE_BUCKET)
